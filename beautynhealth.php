...
month FROM `posts` WHERE category = 'beauty-and-health'
...
if(isset($_GET['query'])) {
  ...
  $sql = "SELECT * FROM posts WHERE category = 'beauty-and-health' AND year(published_at) = '{$_GET['year']}' AND MONTHNAME(published_at) = '{$_GET['month']}' LIMIT {$limit} OFFSET {$offset}";
} else {
    $page_count_sql = "SELECT * FROM posts WHERE category = 'beauty-and-health'";
